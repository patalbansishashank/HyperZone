...
def _output_selector(spec):
    ...
    connector names are assigned per GPU-enumeration order, so they migrate to a
...
def lua_monitor(spec):
    ...
    if spec.get("disabled"):
        return 'hl.monitor({output="%s", mode="disable"})' % _output_selector(spec)
    ...
    parts = ['output="%s"' % _output_selector(spec),
...
class Daemon(HyperZone):
    ...
    def on_cmd(self, msg):
        ...
        if cmd == "retile":
            ...
            self.cmd_locate(active)               # balloon the pointer + pulse the focused border
